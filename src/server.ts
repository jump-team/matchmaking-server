...
app.post("/:type/findmatch", function(request: Request, response: Response) {
  ...
  if (matchTypes[request.params.type]) {
    ...
    if (!bestMatch instanceof Match) {
      response.status(201).json({ match_found: false, retry: true })
    } else {
      response.status(200).json(bestMatch);
    }
  } else {
    response.status(404).json({ match_found: false, retry: true });
  }
});

app.get("/:type/:id" function(request: Request, response: Response) {
  const info = request.body;
  if (matchTypes[request.params.type]) {
    const matches = matchTypes[request.params.type]
    if (matches[request.params.id]) {
      response.status(200).json(matches[request.params.id]);
    }
  }
});
